...
pl_xgb <- po("encode") %>>%
  po(lrn_xgboost)
#traning#
lrn_baseline$train(hearts_task)
lrn_cart$train(hearts_task)
lrn_cart_cp$train(hearts_task)
lrn_xgboost$train(hearts_task2)
...
points(classif.fnr ~ classif.fpr, instance$result[2], col = "green")
points(classif.fnr ~ classif.fpr, instance$result[4], col = "blue")
plot(classif.acc ~ classif.ce, instance$archive$data)
points(classif.acc ~ classif.ce, instance$result, col = "red")
points(classif.acc ~ classif.ce, instance$result[2], col = "green")
points(classif.acc ~ classif.ce, instance$result[4], col = "blue")
#train it on the dataset#
learner_rf4 = lrn("classif.ranger", predict_type = "prob", importance = "permutation")
learner_rf4$param_set$values <- instance$result_learner_param_vals[[4]]
